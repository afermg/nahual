...
- [CellSAM](https://github.com/afermg/cellSAM): Foundation model for cell segmentation. Two branches:
  - `nahual-wrap-onnx` (recommended) — ONNX-only, no auth. Backed by [keejkrej/cellsam-onnx](https://huggingface.co/keejkrej/cellsam-onnx) (image_encoder + cellfinder + mask_decoder + image_pe). Weights auto-downloaded into `~/.cache/cellsam-onnx/` on first `setup()`. License is *Modified Apache 2.0, academic-only* (commercial use → contact `vanvalenlab@gmail.com`). Run with `nix run github:afermg/cellSAM/nahual-wrap-onnx -- ipc:///tmp/cellsam.ipc`.
  - `nahual-wrap` — original PyTorch path. Requires `DEEPCELL_ACCESS_TOKEN` (issued at https://users.deepcell.org) to fetch weights; set it before `nix run`. Without the token, the first `setup()` call returns `{}` and the client logs `unpack requires a buffer of 2 bytes`.
